...
class CfgVehicles {
    ...
    class tib_flag_gdi_01_f : FlagCarrier {
        scope = 2;
        scopeCurator = 2;
        displayName = CSTRING(gdi_01);
        author = ECSTRING(main,modteam);
        //editorPreview = QPATHTOF(EditorPreviews\tib_flag_gdi_01_f.jpg);

        class EventHandlers {
            init = (_this select 0) setFlagTexture QPATHTOF(data\tib_flag_gdi_01_co.paa);
        };
    };
    class tib_banner_gdi_01_f : Banner_01_base_F {
        scope = 2;
        scopeCurator = 2;
        displayName = CSTRING(Banner_GDI_01);
        author = ECSTRING(main,modteam);
        //editorPreview = QPATHTOF(EditorPreviews\tib_Banner_GDI_01_F.jpg);

        hiddenSelectionsTextures[] = {
            QPATHTOF(data\tib_flag_gdi_01_co.paa)
        };
    };
    ...
    class tib_flag_nod_01_f : FlagCarrier {
        ...
        author = ECSTRING(main,modteam);
        //editorPreview = QPATHTOF(EditorPreviews\tib_flag_nod_01_f.jpg);

        class EventHandlers {
            init = (_this select 0) setFlagTexture QPATHTOF(data\tib_flag_nod_01_co.paa);
        };
    };
